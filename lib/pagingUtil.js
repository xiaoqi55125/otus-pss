...
exports.commonPagingProcess = function (sqlWithoutPaging, sqlCondition, pagingCondition, callback) {
    if (!sqlWithoutPaging) {
        return callback(new Error("paging sql can not be empty"), null);
    }
    ...
    if (needPaging) {
        params.START = pagingCondition.start || 0;
        params.END   = pagingCondition.end || config.default_page_size;
    }
...
function handleSql (querySql) {
    ...
    var hasGroupInOriginalSql = (querySql.toLowerCase().indexOf("group") != -1);

    if (!hasGroupInOriginalSql) {
        countSql = " select count(1) as totalNum " + querySql.substring(fromIndex);
    } else {                                                    //handle GROUP sub statement
        countSql = " select count(1) as totalNum from ( " + querySql + " ) tmp"
    }
